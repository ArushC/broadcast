...
public class BasicIBBESystem {
	...
	public static Object[] setup(int lambda, int m) {
		...
		BasicIBBESystem.m = m; //save so it can be used for other functions
	...
	//SUCCEED
	public static void testYComputation(ArrayList<Fr> S, Fr ID) {
		
		ArrayList<Fr> y = computeY(S);
		Fr sum = new Fr(0);
		Fr exponentiated = new Fr(ID);
		for (int i = 1; i <= m; i++) {
			Fr yi = y.get(i);
			Fr product = new Fr();
			Mcl.mul(product, yi, exponentiated);
			Mcl.add(sum, sum, product);
			Mcl.mul(exponentiated, exponentiated, ID);
		}
		Mcl.mul(sum, sum, new Fr(-1));
