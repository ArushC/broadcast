...
public class NonriskyMTBRevised {
	...
	public static Object[] genMTB(int u, int v, int t, int n, int lambda) {
		...
		NonriskyMTBRevised.u = u;
		NonriskyMTBRevised.t = t;
		NonriskyMTBRevised.n = n;
		NonriskyMTBRevised.v = v;
		MAX_HELPER_KEYS = v;
	...
	//encryption happens to the jth instance. In each instance, only the users in that instance with i-values in the subset S can decrypt
		public static void printRuntimes(int N, int M, int subsetSize, int lambda) {
			...
			Object[] setup = NonriskyBroadcastMultischeme.setup(N, M, Mcl.BN254);
			...
			Object[] enc = NonriskyBroadcastMultischeme.enc(PK, j, S);
			...
			Object[] skID = NonriskyBroadcastMultischeme.keyGen(ID,  tk);
			...
			GT encapsulatedKey1 = NonriskyBroadcastMultischeme.decrypt(skID, S, ID, c);
...
//this is the nonrisky broadcast & trace scheme given in section 9.3 of https://eprint.iacr.org/2020/954
//note: page 36 is helpful
class NonriskyBroadcastMultischeme {
